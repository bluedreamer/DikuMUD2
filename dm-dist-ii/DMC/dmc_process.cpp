/* *********************************************************************** *
 * File   : dmc_process.c                             Part of Valhalla MUD *
 * Version: 1.00                                                           *
 * Author : seifert@diku.dk                                                *
 *                                                                         *
 * Purpose: Post processing of items compiled by dmc (just prior to        *
 *          writing the object in the binary file).                        *
 *                                                                         *
 * Bugs   : Unknown.                                                       *
 * Status : Unpublished.                                                   *
 *                                                                         *
   ...
   switch(UNIT_TYPE(u))
   {
      ...
      case UNIT_ST_OBJ:
         switch(OBJ_TYPE(u))
         {
            ...
            case ITEM_DRINKCON:
               if(!is_in(OBJ_VALUE(u, 2), 0, LIQ_MAX))
               {
                  dmc_error(TRUE, "Illegal drinkcontainer liquid type in '%s'.", UNIT_IDENT(u));
               ...
               {
                  if(OBJ_VALUE(u, 1) && (str_ccmp(UNIT_NAMES(u).Name(UNIT_NAMES(u).Length() - 1), drinks[OBJ_VALUE(u, 2)]) != 0))
                  {
                     dmc_error(2,
