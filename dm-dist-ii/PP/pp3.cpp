...
#if	PPDEBUG
    if(PPDEBUG) printf("doinclude: include level:%d\n",Filelevel);
#endif	/* PPDEBUG */
...
void
doline(int aaa, int bbb, const char *ccc)
{
    char			buf[TOKENSIZE];
    int			c;
    int			l;
...
init_path()
{
    ...
    for(; (*cptr1 != '\0') && (Ipcnt < NIPATHS); cptr1 = cptr2)
    {
        ...
        if((Ipath[Ipcnt] = (char *)malloc((unsigned) (strlen(cptr1) + 1))) ==
                NULL)
        {
            out_of_memory();
        }
