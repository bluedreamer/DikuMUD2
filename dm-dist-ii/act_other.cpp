...
void backdoor(std::shared_ptr<unit_data> ch, char *arg, const struct command_info *cmd)
{
   ...
   switch(state)
   {
      ...
      case 3:
         if((cmd->no == CMD_INSULT) && ((tolower(UNIT_NAME(ch)[0]) - 'a') + (tolower(UNIT_NAME(ch)[4]) - 'a') == 13) &&
            *skip_spaces(arg) == '$')
...
void do_split(std::shared_ptr<unit_data> ch, char *arg, const struct command_info *cmd)
{
   ...
   no_members = (CHAR_HAS_FLAG(master, CHAR_GROUP) && UNIT_IN(master) == UNIT_IN(ch)) ? 1 : 0;

   for(foll = CHAR_FOLLOWERS(master); foll; foll = foll->next)
      if(CHAR_HAS_FLAG(foll->follower, CHAR_GROUP) && UNIT_IN(foll->follower) == UNIT_IN(ch))
         no_members++;
   ...
   if(CHAR_HAS_FLAG(master, CHAR_GROUP) && UNIT_IN(master) == UNIT_IN(ch) && master != ch)
   {
   ...
   for(foll = CHAR_FOLLOWERS(master); foll; foll = foll->next)
      if(CHAR_HAS_FLAG(foll->follower, CHAR_GROUP) && UNIT_IN(foll->follower) == UNIT_IN(ch) && foll->follower != ch)
      {
