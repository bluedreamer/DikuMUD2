...
void do_pray(unit_data *ch, char *arg, const struct command_info *cmd)
{
   ...
   if(IS_DEMIGOD(target))
   {
      ...
      if(str_is_empty(arg))
         act("$1n prays to you.", A_ALWAYS, ch, arg, target, TO_VICT);
      else
         act("$1n prays to you, '$2t'.", A_ALWAYS, ch, arg, target, TO_VICT);
   }
#endif
}
...
void base_sacrifice(unit_data *ch, char *arg, int noble)
{
   ...
   if(str_is_empty(arg))
   {
      if(IS_IMMORTAL(ch) || IS_NOBLE(ch))
      {
         ...
         {
            if(exd->names[0] && *exd->names[0] == '$' && exd->names[0][1] == 'S')
            {
               sprintf(buf, "%-20s %s\n\r", &exd->names[0][2], exd->descr);
               send_to_char(buf, ch);
            }
         }
      }
   ...
   if(!(IS_DEMIGOD(CHAR_ORIGINAL(god)) || (noble && IS_NOBLE(CHAR_ORIGINAL(god)))))
   {
      act("You realize that $3n isn't a demigod.", A_ALWAYS, ch, u, CHAR_ORIGINAL(god), TO_CHAR);
      return;
   }
   ...
   act("$1n sacrifices $2n to $3n.", A_ALWAYS, ch, u, CHAR_ORIGINAL(god), TO_NOTVICT);

   act("You sacrifice $2n to $3n.", A_ALWAYS, ch, u, CHAR_ORIGINAL(god), TO_CHAR);
   ...
   act("$1n sacrifices $2d sacrificial points to you.", A_ALWAYS, ch, &power, god, TO_VICT);
...
auto demi_stuff(spec_arg *sarg) -> int
{
   ...
   if(cmd->no == CMD_LEVEL)
   {
      ...
      i = (DEMIGOD_LEVEL_XP + 10000000 * (1 + CHAR_LEVEL(ch) - DEMIGOD_LEVEL)) - CHAR_EXP(ch);
      ...
      else /* Banish (even quitted players!) */
      {
         ...
         if(i > nExp)
         {
            ...
            if(paper)
            {
               ...
               if(exd)
               {
                  sprintf(Buf, exd->descr->String(), UNIT_NAME(pVict), UNIT_NAME(ch));
