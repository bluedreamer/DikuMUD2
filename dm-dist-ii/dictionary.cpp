/* *********************************************************************** *
 * File   : dictionary.c                              Part of Valhalla MUD *
 * Version: 2.45                                                           *
 * Author : gnort@daimi.aau.dk                                             *
 *                                                                         *
 * Purpose: Online alias, based around an object.                          *
 *                                                                         *
 * Bugs   : Unknown.                                                       *
 * Status : Unpublished.                                                   *
 *                                                                         *
...
/* This was hell writing. If someone doesn't appreciate it, I'll kill 'im */

/*
 12th Aug 1993: I ought to be done now.
   All that's left is testing...
   Eliminated the input syncronization problem.
     Commands can no longer `overtake' each other.

 28th Apr 1994: Guess I wasn't ;-)
   Added unalias and changed one argument alias to just list the alias.
   Added dictionary-owner and the claim command, because of security-hole.
     (*sigh*  Why do we always have to worry about the arseholes of the world?)
   Added test for CMD_AUTO_SAVE, to make alias-adding a bit more efficient.
   Got around to commenting the damn thing a bit more thoroughly.

 24th May 1994: Hrmf
   Had to add upper bound check for nasty aliases that call other nasty
     aliases in legal ways, but with extensive recursion check overhead.
     (I love `bug' abusers...)

 20th Jul 1994:
   Previous upper bound was only effective on check-time deep aliases.
   Added similar check for run-time deep aliases.

Wed Oct 12 10:46:23 MET 1994: seifert
   Fixed bug that could cause crash if $alias was missing. Line 668
   modified, and no longer different if STATIC_ALIAS / not. Hope it
   is correct. Seems that before there could be a NULL exd.

Mon Nov 14 17:19:47 MET 1994
   Fixed bug when char was extracted / quit / purged etc.

 11th Jan 1995: gnort
   Cleaned up after seiferts above two bug-fixes :-)
   Got rid of STATIC_ALIAS, MAX_ALIAS_COUNT is now always allocated every
     time a dictionary is initialized.  Good or bad?

  7th Mar 1995: gnort
   Hmm, `got rid' of '!' bug...  Maybe it should be more robust and NOT break
     down due to things like new handling of '!'....

 24th Mar 1995: gnort
   Added check, so people can't use shout as a command...

   Papi: My players wanted it, so I just put in an #ifdef for LINUX.

 14th of June 1995: seifert
   Keeps crashing at 761, assert(strlen(tmp) < MAX_ALIAS_COUNT+ALIAS_NAME+2);
   I've seen lengths up to 11 or more too long. I've just expanded the buffer
...
static struct extra_descr_data *fix_extra_descr(struct unit_data *obj, class extra_descr_data *exd)
{
   ...
   if(exd == NULL)
      exd = UNIT_EXTRA_DESCR(obj) = UNIT_EXTRA_DESCR(obj)->add(aliaslist, NULL);
...
static int local_dictionary(struct spec_arg *sarg)
{
   ...
   if((alias_h = (struct alias_head *)sarg->fptr->data) == NULL)
   {
      ...
      sarg->fptr->data = str_to_alias(exd ? exd->descr.String() : NULL);
