...
#define PC_ABI_COST(ch, index)  \
   (UPC(ch)->ability_cost[index])
...
#define PC_SKI_COST(ch, index)  \
  (UPC(ch)->skill_cost[index])

#define PC_WPN_LVL(ch, index)  \
  (UPC(ch)->weapon_lvl[index])

#define PC_WPN_COST(ch, index)  \
  (UPC(ch)->weapon_cost[index])
...
#define PC_SPL_COST(ch, index)  \
  (UPC(ch)->spell_cost[index])
...
#define UNIT_TITLE_STRING(unit) \
   (UNIT_TITLE(unit).String())

#define UNIT_OUT_DESCR_STRING(unit) \
   (UNIT_OUT_DESCR(unit).String())

#define UNIT_IN_DESCR_STRING(unit) \
   (UNIT_IN_DESCR(unit).String())
