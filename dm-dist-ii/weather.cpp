...
void mudtime_strcpy(struct time_info_data *time, char *str)
{
   ...
   sprintf(tmp, ",\n\r   the %d%s Day of the %s, Year %d.\n\r", day, suf, month_name[(int)time->month], time->year);
   strcat(b, tmp);
...
static void weather_and_time_event(void *p1, void *p2)
{
   ...
   event_enq(SECS_PER_MUD_HOUR * 4, weather_and_time_event, nullptr, nullptr);
}
...
void boot_time_and_weather()
{
   ...
   event_enq(PULSE_SEC * SECS_PER_MUD_HOUR, weather_and_time_event, nullptr, nullptr);
}
